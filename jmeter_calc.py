...
def main():
    ...
    if think_time_ms <= 0:
        ...
        think_time = 0.001  # 1 ms
    else:
        think_time = think_time_ms / 1000.0

    if loop_count:
        total_requests = threads * loop_count
        estimated_total_time = ramp_up + (loop_count * think_time)
    ...
    time_resolution = 0.5
    total_steps = int(duration / time_resolution) + 1
    ...
    for t in range(total_steps):
        ...
        elif current_time <= ramp_up + (test_duration or 0):
            active_users = int(threads * (1 - ((current_time - ramp_up) / test_duration)))
